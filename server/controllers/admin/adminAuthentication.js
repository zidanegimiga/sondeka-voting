...
const login = asyncHandler(async (req, res) => {
    ...
    if (!foundUser) {
        return res.status(401).json({ title: 'One Little Problem', description: 'Incorrect details' })
    }

    const match = await bcrypt.compare(password, foundUser.password)

    if (!match) return res.status(401).json({ title: 'One Little Problem', description: 'Incorrect details' })
